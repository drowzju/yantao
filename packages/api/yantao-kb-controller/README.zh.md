...
| `yantaoKb.capabilityList` | `()` | `{ capabilities, unregistered }`——知识库根目录下每个声明了能力清单的技能，每行合并它的持久化记录（`lastRunAt`、`state`），外加 `unregistered` 分组：KB 外可采纳进知识库的技能目录、KB 内声明缺失或无效的技能（灰显行携带 `inKb` 与原因），以及投放进来的插件仓库（顶层无 SKILL.md 但有内嵌 `skills/<child>/SKILL.md`——行携带 `plugin: true` 与 `pluginSkills` 清单；注册时提取内含技能）（ADR-0025 决定 1）；内置能力会先被播种进 `<kbRoot>/.dsh/skills/`（ADR-0021） |
...
| `yantaoKb.capabilityRegister` | `({ name, agentInvoke?, resourceMenu?, selectionMenu? })` | `{ path }`——把一个 KB 内技能注册成能力：原地写指令型 sidecar（`yantao.json`，无 `entry`，不拷贝）；三个布尔是对话框收集的 reach——`agentInvoke` 把 `invocation` 放宽为 `['human','agent']`，`resourceMenu`/`selectionMenu` 分别写 `appliesTo.resource: true`（所有资源）/ `appliesTo.selection: true`；投放的插件仓库（顶层无 SKILL.md、内嵌 `skills/<child>/SKILL.md`）走**提取**注册——每个内含技能移动到 `.dsh/skills/<child>/` 并各写 sidecar（与仓库同名的内含技能拍平进仓库目录本身），撞名时全有或全无地拒绝；拒绝已经是能力的技能（修复无效 sidecar 才是本调用的用途）、`user-invocable: false` 的技能与扁平单文件技能（ADR-0025 决定 1） |
...
| [`tests/capability-rpc.spec.ts`](tests/capability-rpc.spec.ts) | 能力 RPC 基于假注册表与被 mock 的运行器：no-root/not-found/bad-manifest 拒绝、入口限制、产物写盘、状态往返、列表合并、目录注册、脚手架、指令型能力与 sidecar `invocation` 声明 |
| [`tests/capability-tool.spec.ts`](tests/capability-tool.spec.ts) | `kb_run_capability` 工具与 pre-step 目录（ADR-0023）：注册、agent 门、指令型作答、目录注入，基于假注册表 |
