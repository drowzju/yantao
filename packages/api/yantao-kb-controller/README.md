...
| `yantaoKb.capabilityList` | `()` | `{ capabilities, unregistered }` — every skill at the KB root that declares a capability manifest, each row merged with its persisted record (`lastRunAt`, `state`), plus the `unregistered` group: out-of-KB skill directories adoptable into the KB, and in-KB skills whose declaration is missing or invalid (greyed rows carrying `inKb` and the reason) (ADR-0025 决定 1); the shipped capabilities are seeded into `<kbRoot>/.dsh/skills/` first (ADR-0021) |
...
| `yantaoKb.capabilityRegister` | `({ name, entry? })` | `{ path }` — registers one in-KB skill by writing its `yantao.json` sidecar in place (no copy): with `entry` a script capability (`runtime: 'python'`), without one an instruction capability, always `invocation: ['human']`; refuses a skill that is already a capability (repairing an invalid sidecar is the point), a `user-invocable: false` skill, a flat single-file skill, and an `entry` escaping the skill directory (ADR-0025 决定 1) |
