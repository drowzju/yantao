...
| `yantaoKb.capabilityList` | `()` | `{ capabilities, unregistered }` — every skill at the KB root that declares a capability manifest, each row merged with its persisted record (`lastRunAt`, `state`), plus the `unregistered` group: out-of-KB skill directories adoptable into the KB, in-KB skills whose declaration is missing or invalid (greyed rows carrying `inKb` and the reason), and dropped plugin repositories (top level without a SKILL.md but with nested `skills/<child>/SKILL.md` — rows carrying `plugin: true` and the `pluginSkills` list; registration extracts the nested skills) (ADR-0025 决定 1); the shipped capabilities are seeded into `<kbRoot>/.dsh/skills/` first (ADR-0021) |
...
| `yantaoKb.capabilityRegister` | `({ name, agentInvoke?, resourceMenu?, selectionMenu? })` | `{ path }` — registers one in-KB skill by writing an instruction sidecar (`yantao.json`, no `entry`) in place (no copy); the three booleans are the reach the dialog collected — `agentInvoke` widens `invocation` to `['human','agent']`, `resourceMenu`/`selectionMenu` write `appliesTo.resource: true` (every resource) / `appliesTo.selection: true`; a dropped plugin repository (no top-level SKILL.md, nested `skills/<child>/SKILL.md`) is registered by **extraction** — every nested skill moves to `.dsh/skills/<child>/` with its own sidecar (a child named after the repository flattens into the repository itself), all-or-nothing on name collisions; refuses a skill that is already a capability (repairing an invalid sidecar is the point), a `user-invocable: false` skill, and a flat single-file skill (ADR-0025 决定 1) |
