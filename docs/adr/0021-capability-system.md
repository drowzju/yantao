...
6. **发现挂 skill-filesystem，消费层自建**。启用 dsh `skill-filesystem`（目录扫描、chokidar
   ...
   技能来源按 ADR-0024 决定 4 只认 `<kbRoot>/.dsh/skills/`。落地注记 2026-09-15，
   ADR-0025 落地注记三：中央路由文件 `<kbRoot>/.dsh/skills/yantao.json` 成为第二条声明
   通道——有效 sidecar 仍压过同名路由，路由只在 sidecar 缺失/无效时生效；上句由此放宽为：
   没有声明但被中央路由认领的目录，以路由声明的 reach 成为能力。）
...
| # | 牺牲 | 换来 | 重开条件 |
|---|---|---|---|
| 1 | agent 不能自主发现/调用能力（模型自主性为零） | 下载的任意指令不进模型上下文，信任边界仍是七个 `kb_*` 工具 | 能力白名单机制落地后，可对白名单内能力开 `modelInvocable` ——**已由 ADR-0023 重开（2026-09-14）：sidecar `invocation` opt-in 即白名单** |
| 2 | 会话中不能再取数（分析中途无法补拉邮件、重抽取） | agent 无执行下载代码的通道，宿主执行面只有"人点应用"一个入口 | 同第 1 条，且需设计带人工确认的受控执行工具——**已由 ADR-0023 重开：`kb_run_capability` 即受控执行工具** |
...
- 落地注记（2026-09-14，ADR-0023）：`kb_run_capability` 工具落地，取舍台账第 1、2 条重开——
  sidecar `invocation`（缺省 `["human"]`）即白名单，声明了 `"agent"` 的能力才能被模型经该
...
- 落地注记（2026-09-14，邮件分析分级）：mail 能力 sidecar 升到 4，`read_outlook.py` 增发
  `toMe`（`to`/`cc`/`none`/`unknown`——只记所有者与邮件的关系，不落收件人列表，ADR-0019
  立场不变）并把正文上限放宽到 12000 字。分析 prompt 注入 KB 人员实体的 `relation` 与可选
  `email`，三级判定 `focus`/`digest`/`normal`：主送我且严重、或涉 `relation: superior` 者
  重点提醒；邮催、日常通知归入汇总。资源笔记改为「要点在前、原文在后」，原文经提议里的
  邮件序号回填。分块（每 10 封一次 jsonRound）进度逐批回报到邮件面板。
