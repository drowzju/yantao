...
> **修订（2026-09-17）**：**web 壳已砍掉**，第 1 条的「两者并存」与第 8 条的双退出条件就此终结——
> 桌面壳成为唯一形态。浏览器入口的启动/停止脚本（`scripts/yantao-web-{start,stop}.ps1`）已删除。
> 注意砍掉的只是**浏览器入口**：`yantao-web` profile、loopback HTTP 与 `apps/yantao` 前端是桌面壳
> 的底座（第 2、3 条），原样保留，前端仍然只有一份。验收表中未勾的 UI 各条（ADR-0011–0017）
> 描述的是前端能力，桌面壳加载的就是同一个页面，随前端落地自动满足；「连续 14 天未手动打开
> 浏览器」一条以本次决定作结。
...
## 验收表（原为「砍 web 壳之前必须全部勾选」；web 壳已于 2026-09-17 砍掉，见顶部修订）
