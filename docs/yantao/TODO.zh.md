...
| **邮件分析分级(ADR-0021 落地注记 2026-09-14)** —— `read_outlook.py` 增发 `toMe`(`to`/`cc`/`none`/`unknown`——只记所有者与邮件的关系,不落收件人列表,ADR-0019 立场不变),正文上限放宽到 12000 字;分析 prompt 注入 KB 人员实体的 `relation` 与可选 `email`,每封邮件三级判定:`focus`(主送我且严重——涉上级、明确截止时限、需行动或追责;上级以 `relation: superior` 的人物为准)、`digest`(系统邮催与例行通知,合并汇总)、`normal`;批次按 10 封一块逐块送模型(每块一次 jsonRound),面板逐块点亮每封邮件的徽标(重点/汇总/普通),进度行计「已分析 X/Y」;提议卡新增信息性「重点提醒」区块(红色,置于可勾选组之上)与弱化的「日常通知(汇总)」区块;资源笔记要点在前、原文在后(经提议里的邮件编号回填),人物实体新增 `email:` frontmatter 字段(`kb_create_entity` 参数、模板、树清单、落笔端都有),后续批次按地址优先匹配发件人 | `packages/api/yantao-kb-controller/src/capability/builtin/mail/{yantao.json,SKILL.md,scripts/read_outlook.py}` + `src/{index,types}.ts`、`packages/yantao/kb/src/{core,index,templates}.ts`、`packages/client/ui-yantao/src/client/{mail-analysis,mail-apply,proposal,proposal-apply,remote,index}.ts`、`{MailPanel,ProposalCard}.tsx`。验证:controller + ui-yantao 340、kb 132 测试全绿,`build:lib`、client bundle、scoped oxlint 通过 |

| **ebook 能力与读书项目退役(ADR-0020 落地注记 2026-09-14)** —— 删除 `ebook` 内置能力与整条「创建读书项目」流程:内置母本目录删除,播种器新增退役环节(残留的已播种 `.dsh/skills/ebook` 副本先无损备份到 `.yantao/capability-backups/ebook/<时间戳>-retired/` 再删除);第八个工具 `kb_read_resource` 与 kb 包的 `extractPaths`/`readResourceChunk` 移除(七个工具),`kb_create_entity` 去掉 `source:` 参数;客户端删除 `ReadingDialog`/`reading-flow`/`reading-task`、行菜单与只读横幅入口、`extractResource`/`createReadingProject`/`readBook` 三条缝——mail 重新成为唯一内置能力(算上 `kb_run_capability` 共八个 `kb_*` 工具) | `packages/api/yantao-kb-controller/src/capability/{builtin.ts,builtin/}` + `src/{index,types}.ts`、`packages/yantao/kb/src/{core,index,templates}.ts` + `prompt/sections/filesystem.md`、`packages/client/ui-yantao/src/client/{Workbench,remote,proposal,proposal-apply,index}.ts*` + `frame/Frame.tsx` + `editor/ReadOnlyFile.tsx`,删除 `ReadingDialog.tsx`/`reading-flow.ts`/`reading-task.ts`、`packages/yantao/CONTEXT.md`、各 README 对。验证:三包测试全绿,`build:lib`、client bundle、scoped oxlint 通过 |

| **KB 自包含(ADR-0024)** —— 能力状态从废弃的 `~/.dsh/yantao-kb.json` 迁入 `<kbRoot>/.yantao/state.json`(启动时一次性导入;旧文件改名 `.bak`,绝不删除;遗留 `connectors.mail` 水印不迁移);KB 指针变为 `~/.dsh/settings.yaml` 的 `yantao-kb` 命名空间(`setRoot` 写 settings scope,settings watch 重定向 live root,绕过 `$DSH_HOME` 的 `homedir()` 直调消灭);技能只从 `<kbRoot>/.dsh/skills/` 解析——上游发现机制照常找到所有根,yantao 侧按路径过滤 | `packages/yantao/kb/src/{root-store,index}.ts`、`packages/api/yantao-kb-controller/src/{index,types}.ts`。验证:三包测试全绿,`build:lib`、client bundle、scoped oxlint 通过 |

| **三方技能接入(ADR-0025)** —— `capabilityList` 新增 `unregistered` 分组(无 sidecar 的目录束技能,灰显并给原因),第二十个 RPC `capabilityAdopt` 把一个技能拷进 `.dsh/skills/<name>/` 并写缺省的 human-invocable 指令型 sidecar(拒绝已存在目标、`user-invocable: false`、非目录束);`/xxx` 手势落在控制器的 `agent/pre-step`(消息开头的 `/name` 命中指令型 + human-invocable 能力时,注入一条承载 SKILL.md 正文的 `skill-invocation` 用户消息——双消息形态,上游 tool-skill 保持禁用);资源右键对指令型能力发**当前会话**(SKILL.md 正文 + `@path`,无会话则新建 KB 根目录会话);选区右键由 `appliesTo.selection: true` opt-in(SKILL.md 正文 + 选中文字固定拼接,表面:FileEditor 的 textarea 与 MarkdownView 阅读视图,另有固定项「发送到会话」);`('/','capability')` input-trigger 源补全手势(仅菜单——不定义 `matchEnter`/`matchSpace`,回车裁决完全让给 ui-commands) | `packages/api/yantao-kb-controller/src/{index,types}.ts` + `src/capability/run.ts`、`packages/client/ui-yantao/src/client/{session-prompt,capability-gesture,SelectionMenu}.ts*` + `{CapabilityPanel,Workbench,remote}.ts*` + `frame/Frame.tsx` + `editor/{FileEditor,MarkdownView}.tsx` + `index.ts`、`scripts/gen-cordis-catalog.ts`。验证:三包 458 测试全绿,`tsc -b`、scoped oxlint、`verify-cordis-catalog`、translation pairing、client bundle、frontend build 通过 |
