...
4. **Reading view v3 — `[[wiki links]]` and backlinks** — syntax is `[[名字]]` with an optional `[[类型:名字]]`, resolved by the
...
5. **Reading view v4 — Mermaid and local images** — both are paywalled: client bundles are single-file CJS, so mermaid inlines at
