...
4. **Reading view v2 — clickable task checkboxes and a heading outline** — `MarkdownText` renders checkboxes disabled, so a click
   needs event delegation: map "the Nth checkbox" back to "the Nth `- [ ]` line", flip it, and save through `FileEditor`'s
   existing pre-save comparison (a conflict shows the conflict bar, never a silent write). Pin the mapping with a unit test and
   fall back to the source view when it cannot be resolved.
5. **Reading view v3 — `[[wiki links]]` and backlinks** — syntax is `[[名字]]` with an optional `[[类型:名字]]`, resolved by the
   existing `type:name` rules (singular/plural, case-insensitive) and only on a unique hit; **links may not target `resources/`**.
   Rendering rewrites `[[…]]` into ordinary markdown links before `MarkdownText` (it has no extension hook); targets come from a
   new `links(path)` RPC returning `{ outgoing, incoming }`, computed host-side where the tree load already reads every file.
6. **Reading view v4 — Mermaid and local images** — both are paywalled: client bundles are single-file CJS, so mermaid inlines at
