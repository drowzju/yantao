...
| **「添加目录」 retires (ADR-0021 决定 8 amended)** — installing a capability is now copying its directory into `<kbRoot>/.dsh/skills/` (or `~/.dsh/skills`); the `capabilityRegisterDir` RPC, the `capabilityDirs` persistence, and the controller's dedicated `FileSystemSkillProvider` are gone (nineteen RPCs), and the 能力 tab keeps only 「新建能力」 | `packages/api/yantao-kb-controller/src/{index,types}.ts`, `packages/yantao/kb/src/root-store.ts`, `packages/client/ui-yantao/src/client/{CapabilityPanel,Workbench,remote}.tsx` + `frame/Frame.tsx` + `index.ts` |
